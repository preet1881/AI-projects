...
Original file is located at
    https://colab.research.google.com/drive/1OWO_bvTOlBm3aPWviwcp9A1w4jjvgRKH
...
### torch.nn.functional (F): Contains functions for different operations used in neural networks.
...
### deque: Provides a double-ended queue with fast appends and pops from both ends
...
class Network(nn.Module):
  def __init__(self, state_size, action_size, seed=42) -> None:
      ...
      ### torch.manual_seed(seed): Sets the seed for generating random numbers, ensuring reproducibility.
...
"""
### gym.make: Creates an environment given its id.
### env.reset: Resets the environment to an initial state.
### env.step: Executes one time step within the environment.
...
class Agent():
  ...
  def __init__(self, state_size, action_size):
      ...
      ### optim.Adam: Implements the Adam optimization algorithm.
  ...
  def learn(self, experiences, discount_factor):
    ...
    q_expected = self.local_qnetwork(states).gather(1, actions.long())
    ### F.mse_loss: Computes the mean squared error (MSE) loss.
